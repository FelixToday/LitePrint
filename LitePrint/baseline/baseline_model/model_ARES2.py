...
class Trans_WF(nn.Module):
    ...
    def __init__(self, num_classes, num_patch=1):
        ...
        self.profiling = LocalProfiling(in_channels=num_patch)
    ...
    def forward(self, x):
        sliding_size = np.random.randint(0, 1 + x.shape[-1])
...
if __name__ == '__main__':
    net = Trans_WF(num_classes=100, num_patch=8)
    x = np.random.rand(4, 8, 7200)
    ...
    model = net
    summary(model, input_size=(4, 8, 7200), depth=float('inf'), col_names=["input_size", "output_size", "num_params", "kernel_size"])

...
def process_LT_LS(features,pre_idx,now_idx, LT, LS, t_interval):
    """
    处理LT和LS数据并提取特征到feature的指定列

    参数:
        feature: 特征矩阵
        ind: 要填充的列索引
        LT: 包大小数据 {'upload': [], 'download': []}
        LS: 包时间戳数据 {'upload': [], 'download': []}

    返回:
        更新后的特征矩阵
    """
    # 上行包大小分段数量
    upload_sizes = np.array(LS['upload'])
    features[0, pre_idx] = np.sum((upload_sizes >= 0) & (upload_sizes < 512)) if LS['upload'] else 0
    features[1, pre_idx] = np.sum((upload_sizes >= 512) & (upload_sizes < 1024)) if LS['upload'] else 0
    features[2, pre_idx] = np.sum((upload_sizes >= 1024) & (upload_sizes < 1536)) if LS['upload'] else 0

    # 下行包大小分段数量
    download_sizes = np.array(LS['download'])
    features[3, pre_idx] = np.sum((download_sizes >= 0) & (download_sizes < 512)) if LS['download'] else 0
    features[4, pre_idx] = np.sum((download_sizes >= 512) & (download_sizes < 1024)) if LS['download'] else 0
    features[5, pre_idx] = np.sum((download_sizes >= 1024) & (download_sizes < 1536)) if LS['download'] else 0

    # 间隔和簇数量
    t = np.sort(np.concatenate((LT['upload'], LT['download'])))
    delta_t = np.diff(t)
    features[6, now_idx] = now_idx - pre_idx
    features[7, pre_idx] = np.sum(delta_t > t_interval) + 1


    # 新增特征
    # 1. 上行包数量
    features[8, pre_idx] = len(LT['upload'])

    # 2. 下行包数量
    features[9, pre_idx] = len(LT['download'])

    # 3. 上行包大小总和（除以512）
    features[10, pre_idx] = sum(LS['upload']) / 512 if LS['upload'] else 0

    # 4. 下行包大小总和（除以512）
    features[11, pre_idx] = sum(LS['download']) / 512 if LS['download'] else 0

    # 11. 上行包大小方差
    features[12, pre_idx] = np.var(upload_sizes/512) if len(upload_sizes) > 1 else 0

    # 12. 下行包大小方差
    features[13, pre_idx] = np.var(download_sizes/512) if len(download_sizes) > 1 else 0

    # 计算时间间隔（先diff）
    upload_intervals = np.diff(np.array(LT['upload'])) if len(LT['upload']) > 1 else np.array([])
    download_intervals = np.diff(np.array(LT['download'])) if len(LT['download']) > 1 else np.array([])

    # 13. 上行包平均时间间隔
    features[14, pre_idx] = np.mean(upload_intervals) if len(upload_intervals) > 0 else 0

    # 14. 下行包平均时间间隔
    features[15, pre_idx] = np.mean(download_intervals) if len(download_intervals) > 0 else 0

    # 15. 上行包时间间隔方差
    features[16, pre_idx] = np.var(upload_intervals) if len(upload_intervals) > 1 else 0

    # 16. 下行包时间间隔方差
    features[17, pre_idx] = np.var(download_intervals) if len(download_intervals) > 1 else 0

    return features, pre_idx+1
